...
class ReportOrderCustomerSearch extends Model {
	...
	public function search($params)
	{
			...
			$Query->select(['tmp.customer_id','(select telephone from jr_customer WHERE jr_customer.customer_id = tmp.customer_id ) as telephone1', 'tmp.telephone','tmp.user_agent' ,"count(DISTINCT tmp.order_id) as order_count", "MAX(tmp.date_added) as last_date"])->from(['tmp' => $subQuery])->groupBy("tmp.customer_id");
