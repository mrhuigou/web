...
class ShareApplyDrawController extends Controller{
	public function actionIndex(){
		$model=CustomerCommissionDraw::find()->orderBy("id desc");
	...
	public function actionView($id){
		$model=CustomerCommissionDraw::findOne($id);

        $customer_commission_flow = CustomerCommissionFlow::find()->where(['customer_id' => $model->customer_id,'status'=>1])->andWhere(['>','amount',0]);
        //收益
...
//        ]);
//订单
        $orders = Order::find()->where(['source_customer_id' => $model->customer_id, 'order_type_code' => ['normal', 'presell']])->andWhere(['>','total',0])->andWhere(['not in','order_status_id',[6,7]])->orderby('order_id desc')->all();
...
//        ]);
	...
	public function actionDraw(){
		try{
			if(!$model=CustomerCommissionDraw::findOne(['id'=>\Yii::$app->request->post('id'),'status'=>0])){
			...
			if($result=$mch_pay->getResult()){
				if(($result['return_code']=='SUCCESS') && ($result['result_code']=='SUCCESS')){
					...
					$model->save();
					if($flow=CustomerCommissionFlow::findOne(['type'=>'draw','type_id'=>$model->id])){
						$flow->status=1;
						$flow->update_at=time();
						$flow->save();
					}
