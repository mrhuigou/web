	...
	$.fn.scrollFix = function(options) {
		return this.each(function() {
			...
			function resetScroll() {
				setUnfixed();
				ScrollTopOld = opts.container.scrollTop();
			...
			function onScroll() {
				...
				if (opts.bottom != -1) {
					ScrollTop = ScrollTop + opts.container.height() - outerHeight - opts.bottom;
				}
				if ( ScrollTop-ScrollTopOld >= toTop) {
					if (ie6) { //IE6则使用这个样式
			...
			// 当发现调整屏幕大小时，重新执行代码
			opts.container.on("resize", function() {
				if (Timer) {
					clearTimeout(Timer);
				}
