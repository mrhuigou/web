...
$this->title = '商品详情';
?>
<?=\fx\widgets\Header::widget(['title'=>'商品详情'])?>
		...
		<div class="flex-col tc flex-center">
			<div class="flex-item-4 flex-col pt2">
				...
				<a class="flex-item-6 pr" href="<?= \yii\helpers\Url::to(['/cart/index']) ?>">
					<i class=iconfont>&#xe63b;</i>
					<p>购物车</p>
					<em class="info-point pa-rt r5 cart_qty" style="max-width: 22px;"><?= Yii::$app->fxcart->getCount() ?></em>
				</a>
			</div>
...
$('.J_TSaleProp .sku').each(function() {
    ...
    if(selectedObjs.length) {
        ...
        if(sku_datas[selectedIds.join(';')]){
            ...
            //设置属性状态
            $('.sku').each(function() {
                SKUResult[$(this).attr('attr_id')].count ? $(this).removeClass('disabled') : $(this).addClass("disabled");
            });
