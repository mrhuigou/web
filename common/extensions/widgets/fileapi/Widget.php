 ...
 * ```
 * ...
 * echo \common\extensions\widgets\fileapi\Widget::widget([
 *     'name' => 'fileapi',
 *     'settings' => [
 *         'autoUpload' => true
 *     ]
 ...
 * ```
 * ...
 * echo $form->field($model, 'file')->widget('common\extensions\widgets\fileapi\Widget', [
 *     'settings' => [
 *         'autoUpload' => true
 *     ]
 * ]);
...
class Widget extends InputWidget
{
    ...
    {
        return Yii::t('common\extensions\widgets\\' . $category, $message, $params, $language);
    }
    ...
    {
        if (!isset(Yii::$app->i18n->translations['common\extensions\widgets\fileapi']) && !isset(Yii::$app->i18n->translations['common\extensions\widgets\/*'])) {
            Yii::$app->i18n->translations['common\extensions\widgets\fileapi'] = [
                'class' => 'yii\i18n\PhpMessageSource',
                'basePath' => '@common/extensions/widgets/fileapi/messages',
                'fileMap' => [
                    'common\extensions\widgets\fileapi' => 'fileapi.php'
                ],
    ...
    public function registerFiles()
    {
        if (!isset($this->settings['multiple']) || $this->settings['multiple'] === false) {
            if ($this->hasModel() && $this->model->{$this->attributeName}) {
                $this->settings['files'][] = [
                    'src' => Image::resize($this->model->{$this->attributeName},100,100,9),
                    'name' => $this->model->{$this->attributeName},
                    'type' => @exif_imagetype(Image::resize($this->model->{$this->attributeName},100,100,9))?image_type_to_mime_type(exif_imagetype(Image::resize($this->model->{$this->attributeName},100,100,9))):'',
                ];
            }
        }
    }
