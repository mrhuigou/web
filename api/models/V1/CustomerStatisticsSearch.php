...
/**
 * CustomerStatisticsSearch represents the model behind the search form about `api\models\V1\Customer`.
 */
class CustomerStatisticsSearch extends Customer
{
    /**
     * @inheritdoc
     */
    ...
    public function search($params)
    {
		...
		ORDER BY
			sent_time DESC
		LIMIT 1
	) AS sent_time','(select count(order_id) from jr_order WHERE customer_id=tmp.customer_id )AS  order_count','(select count(order_id) from jr_order WHERE sent_to_erp="Y" and customer_id=tmp.customer_id ) AS  sent_order_count','(
		SELECT
			SUM(total)
		FROM
			jr_order
