...
<?php if($model){ ?>
    ...
    <?php foreach($model as $key=>$val){ ?>
        <tr>
            ...
            <td>
               <span >（<?php if(empty($val->author) || $val->author == 'anonymous'){echo '匿名';}else{ echo $val->author;}?> ）</span>
            </td>
        </tr>
        <?php } ?>
    </table>
    ...
    $('body').on('click','#J_Reviews .pagination a',function(){
    $.ajax({
    url:$(this).attr('href'),
    ...
    $this->registerJs($this->blocks['J_Reviews'],\yii\web\View::POS_READY);
    ?>
    <?php }else{ ?>
    <!--没有记录时-->
