...
class ExpressController extends \yii\web\Controller {
	...
	public function actionCheckCard()
	{
		try {
			if (\Yii::$app->request->getIsPost() && \Yii::$app->request->isAjax) {
				if (!$card_no = \Yii::$app->request->post('card_no')) {
					throw new ErrorException('卡号不能为空');
				}
				if (!$card_pwd = \Yii::$app->request->post('card_pwd')) {
					throw new ErrorException('卡密不能为空');
				}
				if ($model = ExpressCardView::findOne(['card_no' => $card_no, 'card_pwd' => $card_pwd])) {
					if (!$model->card->status) {
						throw new ErrorException('此卡已经失效');
					}
					if ($model->status) {
					...
					\Yii::$app->session->set('ExpressCardViewId', $model->id);
					$data = ['status' => 1];
				} else {
					throw new ErrorException('卡号或卡密不正确');
				}
			} else {
