            ...
            if($status=\Yii::$app->request->get('status')){
                ...
                if($model->subject){
                    $details=PromotionDetail::find()->joinWith([
                        ...
                        "jr_promotion_detail.end_date>'".date('Y-m-d H:i:s')."'",'jr_promotion_detail.status=1',"jr_promotion.subject='".$model->subject."'"])->orderBy('jr_promotion_detail.sort_order asc,jr_promotion_detail.priority asc,jr_promotion_detail.promotion_detail_id asc')->all();
                }else{
                    $details=PromotionDetail::find()->where(['and','promotion_id='.$model->promotion_id,"begin_date>'".date('Y-m-d H:i:s',time())."'",'status=1'])->orderBy('sort_order asc,priority asc,promotion_detail_id asc')->all();
                }
                ...
                if($model->subject){
                    ...
                    $details=PromotionDetail::find()
                        ...
                        ])->where(['and','promotion_id='.$model->promotion_id,"begin_date<='".date('Y-m-d H:i:s',time())."'","end_date>='".date('Y-m-d H:i:s',time())."'",'status=1'])->orderBy('sort_order asc,priority asc,promotion_detail_id asc')->all();
                }
            }
