...
<head>
    ...
    <meta name="description" content="每日惠购（www.mrhuigou.com）- 青岛首选综合性同城网购-家润都市生活平台将城市生活分为吃、喝、穿、用、玩五大类，全面覆盖城市人生活的方方面面-家润生活圈开放30余个生活体验版块，发现达人体验、分享网购乐趣-每日惠购为您量身打造高品质、高性价比、24小时上门服务的同城网购体验" />
    ...
    <script type="application/javascript">
        function init_page(){
            setTimeout(function(){
                $(".menu-tab").scrollFix({container:$(".content"),zIndex : 100,distanceTop:45});
            },100 );
            $('#nav').onePageNav({container:$(".content"),currentClass:"cur",scrollThreshold: 1,easing: 'swing',top:-80});
            $("img.lazy").scrollLoading({container:$(".content")});
        }
    </script>
</head>
<?php $config = \api\models\V1\ConfigCustomize::find()->where(['key'=>'home_style','status'=>1])->andWhere(["and", "date_start<'" . date('Y-m-d H:i:s') . "'", "date_end>'" . date('Y-m-d H:i:s') . "'"])->one();
    if($config){
        $style = $config->value;
    }else{
        $style = "";
...
?>
<body onload="init_page()" class="<?php echo $style;?>">
...
<?php $this->beginBody() ?>
<article>
<?=\h5\widgets\Block\Start::widget()?>
<?= $content ?>
    <?php echo \h5\widgets\Block\NewGuyCouponPop::widget();?>
</article>
<div id="pop_sku">
</div>
<!--微信分享的提示层-->
...
<?php
    if(Yii::$app->request->get('sourcefrom')){ ?>
        <?php $this->beginBlock("JSEND")?>
        //autoSourceFrom('<?php echo Yii::$app->request->get('sourcefrom')?>');
        <?php $this->endBlock();$this->registerJs($this->blocks['JSEND'],\yii\web\View::POS_END);?>
    <?php }  ?>
<?php
$this->endBody() ;
?>
