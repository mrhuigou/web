...
<section class="veiwport">
	<ul class="filter blueFilter two mt5 f14">
		<li><a href="<?=Url::to(['/account-recharge'],true);?>">在线充值</a></li>
		<li class="cur"><a href="<?=Url::to(['/account-recharge/card'],true);?>">充值卡充值</a></li>
	</ul>
	<?php $form = ActiveForm::begin();?>
	<div class="whitebg p10 pt20 pb20 mt2">
		<?= $form->field($model, 'card_no',[
			'template' => "{input}<span class=\"vm red\">{error}</span>",
			"inputOptions"=>["maxlength"=>"16","autocomplete"=>"off",'placeholder'=>'充值卡号','class'=>'input linput w'],
		])->label(false) ?>
		<?= $form->field($model, 'card_pin',[
				'template' => "{input}<span class=\"vm red\">{error}</span>",
			    "inputOptions"=>["maxlength"=>"16","autocomplete"=>"off",'placeholder'=>'充值密码','class'=>'input linput w mt10'],
			])->label(false) ?>
