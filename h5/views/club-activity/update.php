...
<div id="foo">
		<header class="bs-bottom whitebg lh44">
			...
			<h2 class="tc f18"><?=$this->title?></h2>
			<div class="pa-rt">
				<button onclick="javascript:$('#activity-form').submit();" class="btn mr10 w50 lh240 bluebtn" data-role="none">保存</button>
			</div>
		</header>
		<section class="veiwport">
			...
			<div class="bd p5 whitebg pr mb10">
				...
				<?php if($model->image){?>
                <img src="<?=\common\component\image\Image::resize($model->image,65,80)?>" alt="上传图片" width="65" height="80" class="bd" id="activity_theme_pic">
				<?php }else{ ?>
					<p class="upload_pic">上传图片</p>
					<img src="" alt="上传图片" width="65" height="80" class="bd dn" id="activity_theme_pic">
				<?php } ?>
				</div>
				<div class="pl50 ml30">
			...
			<a href="#" class="db bdb whitebg lh44 pr fee-tri">
				<h3 class="pl10">费用</h3>
				<div class="pa-rt pr10 gray9"><em><?=$model->fee == 0?'免费':'AA制'?></em><i class="iconfont rightarr f14"></i></div>
				<?=$form->field($model,'fee',['template'=>'{input}'])->hiddenInput()?>
			</a>
			...
			<a href="#" class="db bdb whitebg lh44 pr cata-tri">
				<h3 class="pl10">类别</h3>
				<div class="tit-right pr10 gray9"><em><?=$model->activity_category_title?></em><i class="iconfont rightarr f14"></i></div>
	    ...
	    <?php ActiveForm::end(); ?>
			...
			<section>
				...
				<div id="l-map" style="width:100%;height:60%;position: absolute;"></div>
...
$(function() {
	...
	$(document).ready(function(){
		$("#activityform-begin_datetime").val("<?=$model->begin_datetime?>");
		$("#activityform-end_datetime").val("<?=$model->end_datetime?>");
		$("#activityform-signup_end").val("<?=$model->signup_end?>");
		<?php if($model->alert_time){
			$alert_time = strtotime($model->begin_datetime)-strtotime($model->alert_time);
			$alert_arr = ["900"=>'提前15分钟',"1800"=>'提前30分钟',"7200"=>'提前2小时',"86400"=>'提前一天'];
			if($alert_arr[$alert_time]){?>
				$(".reminder-tri .pa-rt em").text("<?=$alert_arr[$alert_time]?>");
				$(".reminder-tri input").val("<?=$alert_time?>");
			<?php }
		}?>
	});
	...
	// 百度地图API功能
		...
		var map,lat,lng,address,myValue,saved = 0;
		function init_map(){
		map = new BMap.Map("l-map");
		var point = new BMap.Point(<?=$model->lng?$model->lng:'120.369557'?>,<?=$model->lat?$model->lat:'36.094406'?>); // 创建点坐标
		map.centerAndZoom(point,15);                   // 初始化地图,设置城市和地图级别。
		lat = "<?=$model->lat?>";
		lng = "<?=$model->lng?>";
		address = "<?=$model->address?>";
		<?php if($model->lng && $model->lat && $model->address){?>
			var marker_default = new BMap.Marker(point);
			map.addOverlay(marker_default);    //添加标注
			marker_default.enableDragging();
			marker_default.addEventListener("dragend", function(e){
			  lat=e.point.lat;
			  lng=e.point.lng;
			});
		<?php }?>
