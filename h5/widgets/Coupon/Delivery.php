...
class Delivery extends Widget
{
    ...
    public function init()
    {
        ...
        if(($order_delivery_dates=\Yii::$app->request->post('ViewDeliveryForm')) && isset($order_delivery_dates['delivery'][$store->store_id]['date']) && $order_delivery_dates['delivery'][$store->store_id]['date']){
            $delivery_date=$order_delivery_dates['delivery'][$store->store_id]['date'];
        }else{
            $delivery_date=current($method_times)['date'];
        }
        if(($order_delivery_times=\Yii::$app->request->post('ViewDeliveryForm')) && isset($order_delivery_times['delivery'][$store->store_id]['time']) && $order_delivery_times['delivery'][$store->store_id]['time']){
