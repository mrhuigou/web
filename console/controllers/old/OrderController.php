									...
									if ($order->order_status_id == 9) {
										if ($order->orderShipping) {
											if ($open_id = $order->customer->getWxOpenId()) {
												...
												$notice->order($open_id, "http://m.365jiarun.com/order/shipping?order_no=" . $order->order_no, ['title' => '尊敬的家润会员', 'order_no' => $order->order_no, 'status' => '已发出', 'remark' => $message]);
											}
				...
				foreach ($order_datas as $order_data) {
					...
					if ($model = Order::findOne(['order_id' => $order_data['order_id']])) {
						...
						if ($open_id = $model->customer->getWxOpenId()) {
							$message = "";
							$notice = new WxNotice();
							$notice->order($open_id, "http://m.365jiarun.com/order/index", ['title' => '尊敬的家润会员', 'order_no' => $model->order_no, 'status' => '订单超时，系统取消！', 'remark' => $message]);
						}
//						if($model->use_points){
