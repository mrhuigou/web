								...
								if ($model->order_status_id !== $order_status->order_status_id) {
									...
									if ($order->order_status_id == 9) {
										if ($order->orderShipping) {
										    $wechat2 = false;
										    if($order->affiliate_id == 278){//白金每日惠购的订单
										        $wechat2 = true;
                                            }
											if ($open_id = $order->customer->getWxOpenId($wechat2)) {
												$notice = new WxNotice();
//												$message = "亲:您的" . $order->order_no . "订单已发货，每日惠购小哥正在顶风冒雪赶来，您的一缕微笑是我们更好服务的动力，您可在“账户中心”实时跟踪货物地图位置，祝您购物愉快";
												$message = "亲:您的" . $order->order_no . "订单已发货，每日惠购小哥正在赶来，安全你、我、他，无接触配送，请到小区门口或楼下收货，共抗疫情，配送小哥不进入楼宇，如果小区放行，我们非常高兴送到家里，敬请谅解！您可在“账户中心”实时跟踪货物地图位置，祝您购物愉快";
												$notice->order($open_id, "http://m.mrhuigou.com/order/shipping?order_no=" . $order->order_no, ['title' => '尊敬的每日惠购会员', 'order_no' => $order->order_no, 'total' => $order->total, 'status' => '已发出', 'remark' => $message],$wechat2);
											}
					...
					foreach ($order->orderProducts as $OrderProduct) {
						...
						if ($OrderProduct->gift) {
							foreach ($OrderProduct->gift as $gift) {
                                $spCode=$gift->product->store_code;
								$details[] = [
									'ORDERCODE' => "O" . $order->order_id,
                                    'TYPE' => 'product',
									'LINENO' => $lineno,
...
//								}
        ...
        public function actionPaymentNotice()
        {
            ...
            $order_datas = Order::find()->Where(['and', 'order_status_id=1', 'UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(date_added) >=' . $hour * 3600,'UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(date_added) - 180 <=' . $hour * 3600 ])
                ->andWhere(['order_type_code' => ['normal', 'presell']])
                ->orderBy(['order_id' => 'SORT_ASC'])->asArray()->all();
            if ($order_datas) {
                foreach ($order_datas as $order_data) {
                    if ($model = Order::findOne(['order_id' => $order_data['order_id']])) {
                        $wechat2 = false;
                        if($model->affiliate_id == 278){//白金每日惠购的订单
                            $wechat2 = true;
                        }
                        if ($open_id = $model->customer->getWxOpenId($wechat2)) {
                            $message = "";
                            $notice = new WxNotice();
                            $notice->order($open_id, "http://m.mrhuigou.com/order/index", ['title' => '尊敬的每日惠购会员', 'order_no' => $model->order_no, 'total' => $model->total, 'status' => '你有未支付订单,将8分钟之后自动取消,如需购买请及时支付！', 'remark' => $message],$wechat2);
                        }
                    }
                }
            }
