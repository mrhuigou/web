                ...
                foreach ($order_datas as $order_data) {
                    if ($model = Order::findOne(['order_id' => $order_data['order_id']])) {
                        if ($open_id = $model->customer->getWxOpenId()) {
                            $message = "";
                            $notice = new WxNotice();
                            $notice->order($open_id, "http://m.mrhuigou.com/order/index", ['title' => '尊敬的每日惠购会员', 'order_no' => $model->order_no, 'total' => $model->total, 'status' => '你有未支付订单,将8分钟之后自动取消,如需购买请及时支付！', 'remark' => $message]);
                        }
                    }
                }
            }
