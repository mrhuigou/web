...
class CronController extends Controller
{
    ...
    public function init()
    {
        ...
        if ($this->interpreterPath === null) {
            if ($this->isWindowsOS()) {
                ...
                //nix based OS
                $this->interpreterPath = '/usr/bin/env hhvm';
            }
        }
    ...
    {
        $commandUsage = Yii::getAlias('@runnerScript') . '  cron/' ;// $this->getName()
        return <<<RAW
        ...
        foreach ($parameters AS $n => &$repeat) {
            ...
            if ($repeat === '*') {
                ...
                foreach (explode(',', $repeat) as $piece) {
                    $rand_piece = explode('-', $piece, 2);
                    $res_piece=explode('/',$piece,2);
                    if (count($rand_piece) == 2) {
                        $repeatPiece = array_merge($repeatPiece, range($rand_piece[0], $rand_piece[1]));
                    }elseif(count($res_piece)==2) {
                        $repeatPiece=array_merge($repeatPiece,range($dimensions[$n][0], $dimensions[$n][1],$res_piece[1]));
                    }else{
                        $repeatPiece[] = $piece;
                    }
                }
...
//Getting tag list. If empty, string "default" will be used.
    ...
    protected function runCommandBackground($command, $stdout, $stderr)
    {
        ...
        if(!is_dir(dirname($stdout))){
            mkdir(dirname($stdout));
        }
        if ($this->isWindowsOS()) {
            //Windows OS
    ...
    protected function prepareActions()
    {
        ...
        if (!empty($methods)) {
            foreach ($methods as $runCommand => $runSettings) {
                ...
                $action=$runCommand[count($runCommand)-1];
                unset($runCommand[count($runCommand)-1]);
                $command=implode("/",$runCommand);
                $actions[] = array(
                    'command' => $command,
                    'action' => $action,
